...
def write_DISCON(turbine, controller, param_file='DISCON.IN', txt_filename='Cp_Ct_Cq.txt', rosco_vt = {}):
    ...
    file.write('{0:<12d}        ! DAC_Mode          - DAC control mode {{0: no DAC control, 1: steady state DAC Parameter, 2: Proportional DAC control, 2: Cyclic (1P) DAC control}}\n'.format(int(rosco_vt['DAC_Mode'])))
    file.write('{0:<12d}        ! DAC_Type          - DAC Type {{0: TE Flaps, 1: LEMS}}\n'.format(int(rosco_vt['DAC_Type'])))
    ...
    file.write('{:<014.5f}      ! DAC_Param         - Initial or steady state DAC Parameter Value [rad if angle or defualt units if other device]\n'.format(rosco_vt['DAC_Param']))
    file.write('{:<014.8e}      ! DAC_Kp            - Blade root bending moment proportional gain for DAC control [s]\n'.format(rosco_vt['DAC_Kp']))
    file.write('{:<014.8e}      ! DAC_Ki            - DAC displacement integral gain for DAC control [-]\n'.format(rosco_vt['DAC_Ki']))
    file.write('{:<014.5f}      ! DAC_Max           - Maximum (and minimum) DAC Parameter Value [rad if angle or default units for other device]\n'.format(rosco_vt['DAC_Max']))
    file.write('\n')
...
def DISCON_dict(turbine, controller, txt_filename=None):
    ...
    DISCON_dict['DAC_Type']         = int(controller.DAC_Type)
    ...
    DISCON_dict['DAC_Kp']           = controller.Kp_DAC[-1]
    DISCON_dict['DAC_Ki']           = controller.Ki_DAC[-1]
    DISCON_dict['DAC_Max']          = controller.DAC_max
    # ------- Open Loop Control -------
