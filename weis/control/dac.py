...
def runXfoil(xfoil_path, x, y, Re, AoA_min=-9, AoA_max=25, AoA_inc=0.5, Ma=0.0, multi_run=False, MPI_run=False):
    ...
    while runFlag:
        ...
        if np.size(DAC_polar) < 3: # This case is if there are convergence issues at the lowest angles of attack
            plen = 0
            a0 = 0
            a1 = 0
            ...
            a0 = DAC_polar[-1,0] # Maximum AoA in Polar
            a1 = DAC_polar[0,0] # Minimum AoA in Polar
...
class RunXFOIL(ExplicitComponent):
    ...
    def setup(self):
        ...
        if n_DAC > 0: #May need to add error handeling
            self.DAC_Type = DAC_Type = self.options['modeling_options']['ROSCO']['DAC_Type']
        ...
        # Inputs DAC
        self.add_input('span_end',   val=np.zeros(n_DAC),                  desc='1D array of the positions along blade span where the DAC(s) end. Only values between 0 and 1 are meaningful.')
        self.add_input('span_ext',   val=np.zeros(n_DAC),                  desc='1D array of the extensions along blade span of the DAC(s). Only values between 0 and 1 are meaningful.')
        self.add_input('chord_start',val=np.zeros(n_DAC),                  desc='1D array of the positions along chord where the DAC(s) start. Only values between 0 and 1 are meaningful.')
        self.add_input('delta_max_pos', val=np.zeros(n_DAC), units='rad',  desc='1D array of the max device value (i.e. deflection angle, spoiler height, etc.) of the distributed aerodynamic device. (all angles are in rad, but this value could also have other units)')
        self.add_input('delta_max_neg', val=np.zeros(n_DAC), units='rad',  desc='1D array of the min device value (i.e. deflection angle, spoiler height, etc.) of the distributed aerodynamic device. (all angles are in rad, but this value could also have other units)')
        ...
        self.add_input('cl_interp',        val=np.zeros((n_span, n_aoa, n_Re, n_tab)),   desc='4D array with the lift coefficients of the airfoils. Dimension 0 is along the blade span for n_span stations, dimension 1 is along the angles of attack, dimension 2 is along the Reynolds number, dimension 3 is along the number of tabs, which may describe multiple sets at the same station, for example in presence of a DAC.')
        self.add_input('cd_interp',        val=np.zeros((n_span, n_aoa, n_Re, n_tab)),   desc='4D array with the drag coefficients of the airfoils. Dimension 0 is along the blade span for n_span stations, dimension 1 is along the angles of attack, dimension 2 is along the Reynolds number, dimension 3 is along the number of tabs, which may describe multiple sets at the same station, for example in presence of a DAC.')
        self.add_input('cm_interp',        val=np.zeros((n_span, n_aoa, n_Re, n_tab)),   desc='4D array with the moment coefficients of the airfoils. Dimension 0 is along the blade span for n_span stations, dimension 1 is along the angles of attack, dimension 2 is along the Reynolds number, dimension 3 is along the number of tabs, which may describe multiple sets at the same station, for example in presence of a DAC.')

        # Outputs DAC geometry
        self.add_output('span_start', val=np.zeros(n_DAC),                  desc='1D array of the positions along blade span where the DAC(s) start. Only values between 0 and 1 are meaningful.')
        ...
        self.add_output('DAC_Param',      val=np.zeros((n_span, n_Re, n_tab)), units = 'deg',   desc='3D array with the DAC parameter values of the airfoils. Dimension 0 is along the blade span for n_span stations, dimension 1 is along the Reynolds number, dimension 2 is along the number of tabs, which may describe multiple sets at the same station.')
        ...
        # - This is filled if we're not changing the DAC, so we don't need to re-run xfoil every time
        self.saved_polar_data = {}

    def compute(self, inputs, outputs):

        # If trailing edge DAC devices are present, compute the perturbed profiles with XFOIL or general DAC model
        ...
        if self.n_DAC > 0:
            ...
            for i in range(self.n_span):
                # Loop through the DAC specified in yaml file
                for k in range(self.n_DAC):
                    # Only create DAC geometries where the yaml file specifies there is a DAC (Currently going to nearest blade station location)
                    if inputs['s'][i] >= outputs['span_start'][k] and inputs['s'][i] <= inputs['span_end'][k]: 
                        ...
                        if self.DAC_Type == 0:
                            DAC_Param = np.linspace(inputs['delta_max_neg'][k],inputs['delta_max_pos'][k],self.n_tab) * 180. / np.pi
                        else:
                            DAC_Param = np.linspace(inputs['delta_max_neg'][k],inputs['delta_max_pos'][k],self.n_tab)
                        # Loop through the DAC parameter values
                        for ind, fa in enumerate(DAC_Param):
                            if self.DAC_Type == 0: # Only get airfoil coordinates from XFOIL if XFOIL is being used to calculate flap polars
                                ...
                                # self.DAC_profiles[i]['coords'][:,0,ind] = af_DAC.af_DAC_xcoords # x-coords from xfoil file with DAC devices
                                # self.DAC_profiles[i]['coords'][:,1,ind] = af_DAC.af_DAC_ycoords # y-coords from xfoil file with DAC devices
                                # self.DAC_profiles[i]['coords'][:,0,ind] = af_DAC.af_DAC_xcoords  # x-coords from xfoil file with DAC devices and NO gaussian filter for smoothing
                                # self.DAC_profiles[i]['coords'][:,1,ind] = af_DAC.af_DAC_ycoords  # y-coords from xfoil file with DAC devices and NO gaussian filter for smoothing
                                try:
                        ...
                        # # ** The code below will plot the first three DAC profiles (in the case where there are only 3 this will correspond to max negative, zero, and max positive deflection cases)
                        ...
                        # # plt.plot(self.DAC_profiles[i]['coords'][:,0,0], self.DAC_profiles[i]['coords'][:,1,0], 'r',self.DAC_profiles[i]['coords'][:,0,1], self.DAC_profiles[i]['coords'][:,1,1], 'k',self.DAC_profiles[i]['coords'][:,0,2], self.DAC_profiles[i]['coords'][:,1,2], 'b')
                        # plt.plot(self.DAC_profiles[i]['coords'][:, 0, 0],
                        #         self.DAC_profiles[i]['coords'][:, 1, 0], '.r',
                        #         self.DAC_profiles[i]['coords'][:, 0, 2],
                        #         self.DAC_profiles[i]['coords'][:, 1, 2], '.b',
                        #         self.DAC_profiles[i]['coords'][:, 0, 1],
                        #         self.DAC_profiles[i]['coords'][:, 1, 1], '.k')
                        ...
                        # plt.tight_layout()
                        # plt.show()
                        # # # plt.savefig('temp/airfoil_polars/NACA63-self.618_DAC_profiles.png', dpi=300)
                        # # # plt.savefig('temp/airfoil_polars/FFA-W3-self.211_DAC_profiles.png', dpi=300)
                        # # # plt.savefig('temp/airfoil_polars/FFA-W3-self.241_DAC_profiles.png', dpi=300)
                        # # # plt.savefig('temp/airfoil_polars/FFA-W3-self.301_DAC_profiles.png', dpi=300)


        # # ----------------------------------------------------- #
        # # Determine airfoil polar tables blade sections #

        # #  ToDo: shape of blade['profile'] differs from self.DAC_profiles <<< change to same shape
        ...
        #             # create coords using ccblade and calling XFoil in order to be consistent with the DAC method
        #             if flag_coord == 3:
        #                 DAC_Param = 0  # no te-flaps !
        ...
        #             plt.close('all')
        # ------------------------------------------------------------ #
        # Determine airfoil polar tables for blade sections with flaps or LEMS #
        ...
        if self.n_DAC > 0:
            if 'cl_interp_DAC' not in self.saved_polar_data.keys():
                ...
                run_xfoil_params['DAC_Type'] = self.DAC_Type
                # inputs
                ...
                # Multiple processors, but not MPI
                    ...
                    # separate airfoil sections w/ and w/o DAC
                    af_with_DAC = []
                    af_without_DAC = []
                    ...
                    for i, afi in enumerate(af_with_DAC):
                        ...
                        fa_control[afi,:,:] = polars_separate_DAC[i][3]
                    ...
                    for afi in range(self.n_span): # iterate number of radial stations for various airfoil tables
                        cl_interp_DAC_af, cd_interp_DAC_af, cm_interp_DAC_af, fa_control_af, Re_loc_af, Ma_loc_af = get_DAC_polars(run_xfoil_params, afi)
                ...
                Ma_loc = self.saved_polar_data['Ma_loc']       
                    ...
                    #             cl_interp_DAC[afi, :, j, ind] = inputs['cl_interp'][afi, :, j, 0]
                    #             cd_interp_DAC[afi, :, j, ind] = inputs['cl_interp'][afi, :, j, 0]
                    #             cm_interp_DAC[afi, :, j, ind] = inputs['cl_interp'][afi, :, j, 0]

        else:
            for afi in range(self.n_span):
                # for j in range(n_Re):  # ToDo incorporade variable Re capability
                for ind in range(self.n_tab):  # fill all self.n_tab slots even though no DAC devices exist at current radial position
                    c = inputs['chord'][afi]  # blade chord length at cross section
                    rR = inputs['r'][afi] / inputs['r'][-1]  # non-dimensional blade radial station at cross section
        ...
        outputs['DAC_Param']      = fa_control # use vector of DAC parameter controls
...
def get_DAC_polars(run_xfoil_params, afi):
    ...
    DAC_Type            = copy.deepcopy(run_xfoil_params['DAC_Type'])

    # if 'coords' in DAC_profiles[afi]: # check if 'coords' is an element of 'DAC_profiles', i.e. if we have various DAC Parameter Values
    if DAC_Type > 0 and 'DAC_Param' in DAC_profiles[afi]: # check if airfoil polars need to be created using either XFOIL or general_dac_model
        # for j in range(n_Re): # ToDo incorporade variable Re capability
        for ind in range(n_tab):
            #fa = DAC_profiles[afi]['DAC_Param'][ind] # value of respective DAC parameter
            fa_control_af[:,ind] = DAC_profiles[afi]['DAC_Param'][ind] # DAC Parameter vector of distributed aerodynamics control
            ...
            if DAC_Type == 0:
                ...
                print('Run General DAC Model for nondimensional blade span section s = ' + str(round(s,6)) + ' with ' + str(fa_control_af[0,ind]) + ' dac control parameter; Re equal to ' + str(round(Re_loc_af[0,ind],1)) + '; Ma equal to ' + str(round(Ma_loc_af[0,ind],5)))

                clmax_ratio,stall_shift,LD_ratio,alpha0_shift,S_ratio,CD0_shift = LE_Spoiler(fa_control_af[0,ind]) #bem: TODO I need to add in the ability to use the low fidelity TE flap as an option or build in the ability to use different low fidelity models
        ...
        # # ** The code below will plot the three cl polars
            ...
            # plt.figure
            # fig, ax = plt.subplots(1, 1, figsize=(8, 5))
            # plt.plot(np.degrees(run_xfoil_params['aoa']), cl_interp_DAC_af[afi,:,0,0],'r', label='$\\delta_{flap}$ = -10 deg')  # -10
            # plt.plot(np.degrees(run_xfoil_params['aoa']), cl_interp_DAC_af[afi,:,0,1],'k', label='$\\delta_{flap}$ = 0 deg')  # 0
            # plt.plot(np.degrees(run_xfoil_params['aoa']), cl_interp_DAC_af[afi,:,0,2],'b', label='$\\delta_{flap}$ = +10 deg')  # +10
    ...
    else:  # no DAC at specific radial location (but in general 'aerodynamic_control' is defined in blade from yaml)
        for ind in range(n_tab):  # fill all run_xfoil_params['n_tab'] slots even though no DAC devices exist at current radial position
            c = chord[afi]  # blade chord length at cross section
    ...
    return cl_interp_DAC_af, cd_interp_DAC_af, cm_interp_DAC_af, fa_control_af, Re_loc_af, Ma_loc_af
