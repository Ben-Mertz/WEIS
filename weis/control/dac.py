...
class RunXFOIL(ExplicitComponent):
    ...
    def setup(self):
        ...
        if n_dac > 0: #May need to add error handeling
            self.dac_model = dac_model = self.options['modeling_options']['Level3']['ROSCO']['DAC_Model']
        ...
        # Inputs DAC Devices
        self.add_input('span_end',   val=np.zeros(n_dac),                  desc='1D array of the positions along blade span where the distributed aerodynamic device(s) end. Only values between 0 and 1 are meaningful.')
        self.add_input('span_ext',   val=np.zeros(n_dac),                  desc='1D array of the extensions along blade span of the distributed aerodynamic device(s). Only values between 0 and 1 are meaningful.')
        self.add_input('chord_start',val=np.zeros(n_dac),                  desc='1D array of the positions along chord where the distributed aerodynamic device(s) start. Only values between 0 and 1 are meaningful.')
        ...
        self.add_output('span_start', val=np.zeros(n_dac),                  desc='1D array of the positions along blade span where the DAC devices(s) start. Only values between 0 and 1 are meaningful.')
        ...
        # - This is filled if we're not changing the DAC devices, so we don't need to re-run xfoil every time
    ...
    def compute(self, inputs, outputs):
        ...
        #             plt.close('all')
        # ------------------------------------------------------------ #
        # Determine airfoil polar tables for blade sections with aerodynamic control (i.e. flaps or spoilers) #
        ...
        outputs['dac_param']      = dac_control # use vector of dac device parameter controls
        outputs['Re_loc'] = Re_loc
        outputs['Ma_loc'] = Ma_loc

def get_dac_polars(run_xfoil_params, afi):
    '''
    Sort of a wrapper script for runXfoil or general_dac_model - makes parallelization possible
    ...
    if dac_model > 0 and 'dac_param' in dac_profiles[afi]: # check if airfoil polars need to be created using either XFOIL or general_dac_model
        ...
        for ind in range(n_tab):
            ...
            if dac_model == 1:
                print('Run xfoil for nondimensional blade span section s = ' + str(s) + ' with ' + str(dac_control_af[0,ind]) + ' deg flap deflection angle; Re equal to ' + str(Re_loc_af[0,ind]) + '; Ma equal to ' + str(Ma_loc_af[0,ind]))
