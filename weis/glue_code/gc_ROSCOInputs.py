def assign_ROSCO_values(wt_opt, modeling_options, opt_options):
    ...
    if rosco_init_options['DAC_Mode'] > 0:
        try:
            wt_opt['tune_rosco_ivc.DAC_omega']      = rosco_init_options['DAC_omega']
            wt_opt['tune_rosco_ivc.DAC_zeta']       = rosco_init_options['DAC_zeta']
        except:
            raise Exception('If DAC_Mode > 0, you must set DAC_omega, DAC_zeta in the modeling options')
